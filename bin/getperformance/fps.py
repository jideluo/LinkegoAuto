...
class RootFpsData(threading.Thread):
    ...
    def get_frame_data(self):
        ...
        :return:字典
        """
        result = utils.shell("su -c 'service call SurfaceFlinger 1013'").stdout.readline().decode('UTF-8')
        cur_surface = int(result.split("(")[1].split()[0], 16)
        return {"surface": cur_surface, "timestamp": time.time()}

    def run(self):
        user = utils.shell("su -c 'ls'").stdout.readline().decode('UTF-8')  # 检查手机是否可以ROOT用户执行命令
