...
import subprocess
import time
from bin.scriptUtils import exception
import logging
...
def get_app_pid(pkg_name):
    ...
    result = string.decode('UTF-8').split()
...
# 获取设备名称
def get_device_name():
    brandName = shell("cat /system/build.prop | findstr ro.product.brand").stdout.readline().decode('UTF-8').split("=")[1].strip()
    deviceName = shell("cat /system/build.prop | findstr ro.product.model").stdout.readline().decode('UTF-8').split("=")[1].strip()
...
def get_app_uid(pkg_name):
    pid = get_app_pid(pkg_name)
    uid = shell("cat /proc/%s/status|findstr Uid" % pid).stdout.readline().decode('UTF-8').split()[1]
    return uid
